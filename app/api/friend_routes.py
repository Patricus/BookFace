...
def read_friends():
    ...
    Read all friends.
    """
    friends = User.query.join(Friend, Friend.accepted == True).filter(or_(
        and_(User.id == Friend.user_id, Friend.friend_id == current_user.id), and_(User.id == Friend.friend_id, Friend.user_id == current_user.id))).all()
